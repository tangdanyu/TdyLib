...
int VideoRecorder::startRecord(VideoRecordParams videoRecordParams) {
    ...
    LOGE("VideoRecorder 对齐后targetWidth=%d--targetHeight=%d",
...
int VideoRecorder::endRecordAndReleaseResource() {
    if (yuvBufferTotalCount > 0) {
        ...
        LOGE("VideoRecorder yuvBufferTotalCount<=0");
    }
    ...
    if (NULL != videoWriteMutex) {
        ...
        if (NULL != avFormatContext && NULL != avFormatContext->pb) {
            ...
            if (ret != 0) {
                LOGE("VideoRecorder av_write_trailer fail");
            }
        }
    ...
    LOGE("VideoRecorder endRecordAndReleaseResource --success");
    return 0;
}
...
int VideoRecorder::closeStream(BZOutputStream *stream) {
    ...
    if (NULL != stream->avStream)
        LOGE("VideoRecorder closeStream index=%d", stream->avStream->index);
...
int VideoRecorder::addStream(BZOutputStream *stream, AVMediaType mediaType, AVCodecID codec_id) {
    LOGE("VideoRecorder addStream mediaType=%d", mediaType);
    AVCodecContext *avCodecContext;
    /* find the encoder */
    stream->avCodec = avcodec_find_encoder(codec_id);
    if (!stream->avCodec) {
        LOGE("VideoRecorder Could not find encoder for '%s'\n",
                        avcodec_get_name(codec_id));
        return -1;
    }
    stream->avStream = avformat_new_stream(avFormatContext, NULL);
    if (!stream->avStream) {
        LOGE("VideoRecorder Could not allocate stream\n");
        return -1;
    }
    ...
    switch (mediaType) {
        ...
        case AVMEDIA_TYPE_VIDEO:
            LOGE("VideoRecorder video bit_rate=%f", bit_rate / 1024.0f / 1024 / 8);
...
//    av_dict_set(&param, "tune", "zerolatency", 0);
    ...
    if (ret < 0) {
        LOGE("VideoRecorder avCodecContext Could not open video codec: %s\n",
                        av_err2str(ret));
        return -1;
    }
    av_dict_free(&param);
    ...
    if (ret < 0) {
        LOGE("VideoRecorder Could not copy the stream parameters\n");
        return -1;
    }
    ...
    if (!stream->frame) {
        LOGE("VideoRecorder Could not allocate video frame\n");
        return -1;
    }
    stream->tmp_frame = alloc_picture(avCodecContext->pix_fmt, this->targetWidth,
                                      this->targetHeight);
    if (!stream->tmp_frame) {
        LOGE("VideoRecorder Could not allocate temporary picture\n");
        return -1;
    }
    return ret;
}
